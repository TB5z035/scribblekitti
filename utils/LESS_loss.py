...
class LESS_Loss(nn.Module):
    ...
    def forward(self, student_output, student_label,LESS_labels,label_group):
        ...
        loss_propogated = self.supervised_loss(student_output[label_group == 2],propogated_label)
        ...
        loss_weak = torch.sum(torch.log(torch.sum(student_output_weak,dim=1)),dim=0)
